...
const ProductsTabs = ({loadingProductReview, userInfo, submitHandler, rating, setRating, comment, setComment, product}) => {
    ...
    <div className='flex flex-col md:flex-row'>
                    ...
                    {userInfo ? (
                        <form onSubmit={submitHandler}>
                            <div className="my-2">
                                ...
                                <select id='rating' required value={rating} onChange={e => setRating(e.target.value)} className='p-2 border text-white bg-black rounded-lg xl:w-[40rem]'>
                            ...
                            <div className="my-2">
                                ...
                                <textarea id='comment' value={comment} required rows="3" onChange={e => setComment(e.target.value)} className='p-2 border text-white bg-black rounded-lg xl:w-[40rem]'></textarea>
                            </div>

                            <button type='submit' disabled={loadingProductReview} className='bg-pink-600 text-white py-2 px-4 rounded-lg'>Submit</button>
                        </form>
                    ): (
                        <p>Please <Link to='/login'>Sign In</Link> To Give Review</p>
                    )}
                </div>
            )}
        </section>
        ...
        <section>
            {activeTab == 2 && (
                <>
                <div>
                    {product.reviews.length == 0 && <p>No Review</p>}
                </div>
                <div>
